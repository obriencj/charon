...
def upload_post_process(
        ...
        failed_metas: List[str], product_key):
...
def __post_process(
    ...
    if len(failed_files) == 0 and len(failed_metas) == 0:
        ...
        logger.error("%d file(s) occur errors/warnings,"
                     " please see errors.log for details.", total)
        ...
        # wtf?? TODO raise an exception that's caught somewhere
        # sensible
        sys.exit(1)
