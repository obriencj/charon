...
def extract_npm_tarball(path: str, target_dir: str, is_for_upload: bool) -> Tuple[str, list]:
    ...
    Extract npm tarball will relocate the tgz file and metadata files.
        ...
        * Locate tar path ( e.g.: jquery/-/jquery-7.6.1.tgz or
          @types/jquery/-/jquery-2.2.3.tgz).

        * Locate version metadata path (e.g.: jquery/7.6.1 or
          @types/jquery/2.2.3).

    Result returns the version meta file path and is for following
    package meta generating.
    """
    ...
    for f in tgz:
        if f.name.endswith("package.json"):
            ...
            valid_paths.append(os.path.join(tarball_parent_path, basename(path)))
...
def download_archive(url: str, base_dir=None) -> str:
    ...
    # yup, we're using an unverified SSL cert. Shut up about it.
    urllib3.disable_warnings()
    # some versions of requests have their own copy of urllib3
    ...
    local_filename = os.path.join(base_dir, url.split('/')[-1])
